...
2. Regressão coberta:
   - `frontend/src/modules/admin/pages/AdminHubPage.test.tsx` passou a validar a normalização de mensagens persistidas degradadas e o estado atualizado do painel.
3. Validação:
...
## Atualização técnica (2026-03-16) - Home sem contexto eleitoral e Admin resiliente

1. Home/QG:
   - `QgOverviewPage` deixou de mostrar o painel `Contexto eleitoral de referência`.
   - A decisão foi intencional: a Home voltou a ser uma leitura executiva transversal, sem duplicar o eixo nominal já disponível em `Prioridades`, `Insights` e `Eleitorado`.
2. Admin/operação assistida:
   - `routes_ops` passou a normalizar jobs persistidos que permaneceram `queued` ou `running` após reinício da API.
   - Jobs interrompidos agora são marcados como `failed`, recebem log explícito de interrupção e deixam de bloquear os botões da operação assistida.
3. Admin/textos operacionais:
   - `AdminHubPage` passou a normalizar `summary`, `last_message` e `recent_logs` quando ainda chegam com mojibake de execuções antigas persistidas em `ops`.
   - Os cards de Ferramentas operacionais continuam usando `NavIcon` com o mesmo padrão SVG do menu lateral.
4. Validação:
...
## Atualização técnica (2026-03-16) - distribuição territorial do candidato sem truncamento

1. Eleitorado:
   - `frontend/src/modules/electorate/pages/ElectorateExecutivePage.tsx` deixou de limitar a distribuição territorial do candidato a `15` itens e passou a solicitar até `200` locais de votação no recorte.
...
1. Estado oficial da trilha (WIP=1):
   ...
   - `D5` concluído tecnicamente (`BD-050`, `BD-051`, `BD-052`).
   - `D6` concluído tecnicamente (`BD-060`, `BD-061`, `BD-062`).
   - `D7` concluído tecnicamente (`BD-070`, `BD-071`, `BD-072`), incluindo fechamento adicional em `2026-03-03` para explicabilidade e auditoria de pesos.
   - `D8` concluído tecnicamente (`BD-080`, `BD-081`, `BD-082`).
   - pendências históricas de mapa executivo, geocodificação de locais de votação, refatoração das telas QG e estabilização de `QgPages.test.tsx` devem ser consideradas encerradas pelos commits `8dc6c86`, `b2ad30f`, `6c01e12`, `56bea7b` e `eb9a4c6`.
2. Estado funcional consolidado:
   - mapa executivo consolidado em fluxo único (OSM-only), sem branch simplificado paralelo;
   - `/v1/electorate/map` suporta `aggregate_by=polling_place`, com fallback automático para ano eleitoral com dados quando necessário;
   ...
   - geolocalização de locais de votação estabilizada com seed `data/seed/polling_places_diamantina.csv` (`36/36` locais) e auditoria geoespacial dedicada;
   - telas executivas (`Home`, `Prioridades`, `Mapa`, `Insights`, `Cenários`, `Eleitorado`) alinhadas ao contrato atual e com suíte QG estabilizada;
   ...
   - `QgOverviewPage` e `QgPrioritiesPage` agora também exibem contexto eleitoral nominal de referência (ano, cargo principal, liderança e votos válidos) usando `GET /v1/electorate/election-context`;
   - `QgMapPage` agora permite alternar a métrica eleitoral do local e mantém ranking, tooltip e drawer no mesmo contexto;
   - Admin passou a expor operação assistida governada para validação e sincronização do ambiente, usando a mesma rotina oficial do terminal sem criar trilha paralela;
   - o Admin também passou a expor histórico persistido de jobs, autenticação opcional via `x-admin-ops-token` e mensagens explícitas de 403/404 no frontend;
   - referência vigente da refatoração visual/estrutural do mapa está consolidada em `docs/REFATORACAO_TELAS.md`.
3. Validação consolidada mais recente:
   - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_qg_routes.py tests/unit/test_tse_candidate_votes.py tests/unit/test_tse_electorate.py -q` -> `55 passed`.
   - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_ops_routes.py -q` -> `35 passed`.
   ...
   - `npm.cmd --prefix frontend run test -- --run src/modules/admin/pages/AdminHubPage.test.tsx` -> `5 passed`.
   - `npm --prefix frontend run build` -> `OK`.
   - `./.venv/Scripts/python.exe scripts/audit_electorate_consistency.py` -> relatório atualizado.
4. Próximo passo executável real:
   - propagar a mesma leitura nominal já validada para `Insights` e deep-links executivos relacionados, sem abrir `UX-2` antes da hora;
   ...
   - manter `#7` (`CadÚnico/CECAD`) fora do ciclo ativo até desbloqueio externo.
5. Regra de leitura:
   - apenas esta seção define o "próximo passo executável" vigente;
   - seções abaixo preservam histórico de entregas/rodadas e não reabrem pendências já concluídas, salvo regressão comprovada.

## Atualização técnica (2026-03-13) - Home eleitoral resiliente e resumo executivo alinhado ao histórico

1. Correção funcional no backend eleitoral:
   - `GET /v1/electorate/summary` passou a usar o mesmo fallback oficial por granularidade já adotado em `GET /v1/electorate/history`;
   - com isso, anos como `2022` deixam de retornar apenas `total_voters` no resumo e passam a expor também `turnout`, `turnout_rate`, `abstention_rate`, `blank_rate` e `null_rate`.
2. Correção funcional no frontend do eleitorado:
   - `ElectorateExecutivePage` passou a reconciliar o card `Resumo executivo` com a linha do mesmo ano em `Histórico eleitoral` quando o payload de resumo vier incompleto;
   - o resumo também passou a exibir explicitamente `Comparecimento`, reduzindo a inconsistência entre o bloco executivo e a tabela histórica.
3. Hardening na Home/Prioridades:
   - `QgOverviewPage` e `QgPrioritiesPage` passaram a resolver o contexto eleitoral nominal com fallback controlado para `municipality` quando o nível filtrado não trouxer payload nominal consistente;
   - isso reduz o risco de erro isolado no painel `Contexto eleitoral de referência` sem reabrir a modelagem nominal.
4. Validação:
...
## Atualização técnica (2026-03-12) - Hotfix do Admin operacional e limpeza residual de acentuação

1. Correção funcional no frontend operacional:
   - `frontend/src/modules/ops/pages/OpsRunsPage.tsx` teve os rótulos residuais corrigidos (`Execuções de pipeline`, `Início`, `Duração`, `Página`, `Próxima`), removendo a degradação visual que ainda aparecia na navegação operacional.
   - `frontend/src/modules/admin/pages/AdminHubPage.tsx` permaneceu alinhado ao contrato de operação assistida, preservando os estados explícitos de `404`/`403` no painel.
2. Regressão do fluxo central:
   - `frontend/src/app/e2e-flow.test.tsx` foi ajustado para validar elementos estáveis do fluxo executivo atual em `Território 360` e `Cenários`, evitando asserts frágeis em textos narrativos não estruturais.
   - `frontend/src/app/router.smoke.test.tsx` continuou cobrindo a navegação entre `Visão Geral`, `Mapa`, `Prioridades`, `Insights`, `Cenários`, `Eleitorado`, `Briefs` e `Admin`.
3. Documentação e encoding:
   - `docs/HANDOFF.md` e `docs/CHANGELOG.md` tiveram os trechos mais recentes revisados para remover headings/bullets com `?` literal.
   - `scripts/fix_docs_encoding.py` foi reaplicado após a revisão manual e o guardrail de documentação voltou a passar.
4. Próximo passo operacional:
   - sincronizar esse delta para o clone oficial em `D:` e reiniciar o runtime que o usuário efetivamente utiliza, eliminando o cenário em que a UI aponta para um backend/frontend defasados.
5. Validação:
...
## Atualização técnica (2026-03-12) - Operação assistida no Admin

1. Backend operacional:
   - `src/app/api/routes_ops.py` passou a expor execução assistida em background com `POST /v1/ops/admin/sync/start`, `GET /v1/ops/admin/sync/status`, `GET /v1/ops/admin/sync/history` e `GET /v1/ops/admin/sync/jobs/{job_id}`;
   - o gerenciador controla um job administrativo por vez, com etapas, logs recentes, status do passo atual e bloqueio de concorrência para evitar equalizações sobrepostas;
   - o histórico agora fica persistido em `ops.admin_sync_jobs` e `ops.admin_sync_job_steps`, materializados por `db/sql/020_admin_sync_jobs.sql`, fechando a perda de contexto após restart do backend;
   - o backend passou a aceitar `x-admin-ops-token` quando `ADMIN_OPS_TOKEN` estiver configurado, evitando expor a rotina de equalização sem governança fora do ambiente local.
2. Fluxos governados:
   - modo `validate` roda somente checks oficiais sem escrita (`export_data_coverage_scorecard`, `audit_polling_places_geolocation`, `backend_readiness`);
   - modo `sync` dispara a rotina oficial `scripts/equalize_database_env.ps1`, preservando `-IncludeWave7` e `-AllowBackfillBlocked` como defaults operacionais homologados.
3. Frontend Admin:
   - `AdminHubPage` passou a exibir painel de operação assistida com botões `Validar ambiente` e `Sincronizar ambiente`;
   - a sincronização exige confirmação explícita antes de iniciar;
   - o painel mostra status do último job, etapa atual, tabela de etapas, logs recentes e polling automático enquanto houver job ativo;
   - os estados de erro agora diferenciam `404` (backend sem suporte ao recurso) e `403` (token administrativo ausente ou inválido), com mensagens explícitas no painel.
4. Validação:
   ...
   - `npm.cmd --prefix frontend run build` -> `OK`;
   - smoke real:
     - `GET /v1/ops/admin/sync/status` -> `200`, `{ "job": null }` em banco limpo;
     - `GET /v1/ops/admin/sync/jobs/{uuid}` -> `200`, `{ "job": null }` para job inexistente.
## Atualização técnica (2026-03-12) - Histórico eleitoral e seções nominais esclarecidas

1. Histórico eleitoral executivo:
   - `GET /v1/electorate/history` passou a calcular comparecimento, abstenção, brancos e nulos por ano com fallback explícito de granularidade oficial;
   - para anos em que `municipality` não possui consolidação em `silver.fact_election_result`, a API agora utiliza `electoral_zone`, preservando a série histórica da UI sem duplicar contagens.
2. Distribuição territorial do candidato:
   - `GET /v1/electorate/candidate-territories` passou a expor `polling_place_section_count` e `polling_place_sections` no agregado por local de votação;
   - a diferenca entre `Distribuicao territorial do candidato` e `Ranking de locais de votacao` foi formalizada:
     - ranking = todas as seções do local;
     - distribuição do candidato = somente as seções em que o candidato recebeu votos.
3. UX da coluna `Seções`:
   - `ElectorateExecutivePage` passou a usar o mesmo layout em bloco do ranking de locais, com linha principal e preview secundário;
   - quando o candidato tem votos em apenas parte do local, a UI agora explicita `X de Y seções com votos`, reduzindo ambiguidade como no caso da `E. E. PROF.ª GABRIELA NEVES`.
4. Validação:
   - `.\\.venv\\Scripts\\python.exe -m pytest tests/unit/test_qg_routes.py -q` -> `33 passed`;
   - `npm.cmd --prefix frontend run test -- --run src/modules/electorate/pages/ElectorateExecutivePage.test.tsx` -> `8 passed`;
   - `npm.cmd --prefix frontend run build` -> `OK`.

## Atualização tecnica (2026-03-12) - Partido derivado no eleitorado nominal

1. Lacuna fechada na base:
   - o bruto `tse_votacao_secao` materializado neste ambiente nao expõe `SG_PARTIDO`, `NR_PARTIDO` nem `NM_PARTIDO`;
   - para evitar que `silver.dim_candidate` continue nulo nesses campos, foi introduzido o modulo `src/pipelines/tse_party_registry.py`, que deriva partido a partir:
     - do prefixo do numero do candidato;
     - dos votos de legenda já presentes na base nominal do mesmo ano;
     - de um registro histórico controlado para sigla/nome partidario.
2. Pipeline e rotina operacional:
   - `src/pipelines/tse_candidate_votes.py` agora enriquece `party_abbr`, `party_number` e `party_name` antes do upsert;
   - `scripts/backfill_candidate_party_identity.py` foi adicionado para corrigir o estoque já carregado em `silver.dim_candidate`;
   - `scripts/equalize_database_env.ps1` passou a executar esse backfill automaticamente após `cleanup_candidate_vote_zone_legacy`.
3. Efeito funcional validado:
   - `GET /v1/electorate/election-context?year=2024&office=Prefeito` passou a expor `MDB` e `REDE` nos candidatos principais;
   - `GET /v1/electorate/election-context?year=2022&office=PRESIDENTE` passou a expor `PT`, `PL`, `MDB`, `PDT` e demais partidos no contexto nominal;
   - a correcao vale tambem para a serie municipal de `2020`, removendo a lacuna visual de partido na aba `Eleitorado`.
4. Validação:
   ...
   - smoke real:
     - `GET http://127.0.0.1:8000/v1/electorate/election-context?year=2024&office=Prefeito` -> partidos preenchidos;
     - `GET http://127.0.0.1:8000/v1/electorate/election-context?year=2022&office=PRESIDENTE` -> partidos preenchidos.

## Atualização técnica (2026-03-12) - Sincronização do ambiente local de banco

1. Correção estrutural do banco neste computador:
   - `scripts/init_db.py` executado com sucesso (`Applied 21 SQL scripts`) para materializar o schema SQL oficial faltante, incluindo `silver.dim_election`.
2. Equalização operacional:
   - `scripts/equalize_database_env.ps1 -IncludeWave7 -AllowBackfillBlocked` foi reexecutado com sucesso material nas etapas de connector registry, schema contracts, reprocessamento TSE 2024, backfill robusto, cleanup do legado nominal e incremental full sources.
   - as únicas fontes que permaneceram como `blocked` no incremental foram `cecad_social_protection_fetch` e `censo_suas_fetch`, mantendo o comportamento esperado para dependências externas fora do ciclo ativo.
3. Seed e geolocalização:
   - `scripts/build_seed.py` recompilou `data/seed/polling_places_diamantina.csv` neste ambiente;
   - `scripts/apply_seed.py` reaplicou o seed com verificação final de `36` pontos únicos para `286` seções;
   - `scripts/audit_polling_places_geolocation.py --output-json data/reports/polling_places_geolocation_audit.current_env.json` retornou `status=pass` sem violações de distrito.
4. Readiness final:
   - `scripts/export_data_coverage_scorecard.py --output-json data/reports/data_coverage_scorecard.current_env.json` -> `pass=29`, `warn=3`;
   - `scripts/backfill_missing_pipeline_checks.py --window-days 7 --apply` preencheu o run legado sem checks (`d431b7b6-f6aa-4b5d-8892-38f342a935cf`);
   - `scripts/backend_readiness.py --output-json` voltou para `READY`, `hard_failures=0`, `warnings=0`.
5. Hardening aplicado:
   - `scripts/equalize_database_env.ps1` foi ajustado para enviar o `stdout` dos subprocessos para `Out-Host` e preservar apenas o inteiro de `$LASTEXITCODE`, eliminando o falso-aborto observado quando a etapa incremental terminava com `exit code 0`.

## Atualização técnica (2026-03-12) - Contexto eleitoral nominal em Home e Prioridades

1. Frontend executivo:
   - `QgOverviewPage` passou a exibir painel de contexto eleitoral nominal com:
     - ano eleitoral de referência;
     - cargo principal;
     - liderança nominal;
     - total de votos válidos;
     - top 3 candidatos do recorte.
   - `QgPrioritiesPage` passou a exibir resumo nominal no topo, alinhado ao `level` filtrado.
2. Decisão de escopo:
   - a propagação foi feita como camada de leitura executiva, sem alterar o contrato principal de `priority/list` nem abrir filtro adicional de ano eleitoral nesta rodada;
   - o ano/cargo continuam sendo resolvidos pelo backend a partir do melhor recorte nominal oficial disponível para o nível solicitado.
3. Validação:
   - `npm --prefix frontend run test -- --run src/modules/qg/pages/QgPages.test.tsx` -> `24 passed`;
   - `npm --prefix frontend run build` -> `OK`.
...
1. Auditoria nominal reforçada:
   ...
   - a auditoria passa a registrar cobertura nominal por candidato quando `tse_votacao_secao` estiver materializado no Bronze;
   - quando o Bronze nominal ainda não existe, o relatório agora explicita a ausência em vez de quebrar.
2. Report operacional ampliado:
   - `scripts/backfill_robust_database.py` passou a expor:
     - `candidate_vote`
     - `candidate_vote_by_level`
     - `candidate_vote_office_coverage`
3. Diagnóstico atual do ambiente:
   - `data/reports/electorate_consistency_audit_2024.json` confirmou consistência do eleitorado agregado em `2024`;
   - a camada nominal continua apenas em `electoral_zone`;
   - a cobertura atual do banco é:
   ...
   - o Bronze `data/bronze/tse/tse_votacao_secao` ainda não existe neste ambiente.
4. Próximo passo executável real:
   - materializar `data/bronze/tse/tse_votacao_secao` neste ambiente;
   - executar backfill `2016-2024` do nominal por seção;
   - rerodar a auditoria para confirmar migração de `electoral_zone` para `electoral_section` e derivação consistente de `local de votação`.

## Atualização técnica (2026-03-08) - Backfill nominal por seção concluído

1. Carga histórica executada:
   - `tse_candidate_votes_fetch` rodou com sucesso para `2016`, `2018`, `2020`, `2022` e `2024`;
   - Bronze `tse_votacao_secao` foi materializado para os cinco anos neste ambiente.
2. Cobertura nominal atual do banco:
...
3. Efeito funcional:
   - `GET /v1/electorate/election-context` agora resolve o recorte municipal com `source_level=electoral_section`;
   - `GET /v1/electorate/candidate-territories?aggregate_by=polling_place` já responde com ranking nominal por local de votação.
4. Residual/atenção:
   - permanecem `165` linhas legadas em `electoral_zone` para `2024`;
   - as eleições gerais `2018` e `2022` ainda precisam de auditoria específica para confirmar se o pacote `MG` basta ou se será necessário complemento `BR` para cargos ausentes.
5. Evidências:
   - `data/reports/tse_candidate_votes_section_direct_run.json`
...
1. Backend/API:
   - `GET /v1/electorate/candidate-territories` agora devolve `candidate_territories_unavailable|source_level=electoral_zone|requested_aggregate=polling_place` quando a carga nominal real só existe em `zona eleitoral`.
   - `GET /v1/electorate/election-context` segue expondo `source_level=electoral_zone` no agregado municipal para `2024`.
2. Frontend executivo:
   - `ElectorateExecutivePage` deixou de mostrar estado vazio genérico para voto nominal por candidato quando a granularidade não existe.
...
4. Próximo passo executável real:
   ...
   - manter o aviso de granularidade até existir fonte nominal por `local de votação` ou `seção eleitoral`.

## Atualização técnica (2026-03-07) - Estudo de viabilidade TSE por seção/local

1. Estudo concluído:
   - `docs/ESTUDO_TSE_SECAO_LOCAL_VOTACAO.md` consolidou a verificação local + internet.
   - o TSE publica `votacao_secao` para `2016`, `2018`, `2020`, `2022` e `2024`.
2. Evidência estrutural confirmada:
   - os arquivos oficiais de MG contêm, de forma consistente:
     - `NR_SECAO`
     - `NR_LOCAL_VOTACAO`
     - `NM_LOCAL_VOTACAO`
...
3. Conclusão operacional:
   - a limitação atual em `zona eleitoral` não é limitação do TSE para a série histórica ativa;
   - a limitação atual decorre do conector `tse_votacao_candidato_munzona`.
4. Reorientação da trilha:
   - o próximo passo correto deixa de ser propagar a camada nominal atual para mais telas;
   - o próximo passo correto passa a ser migrar o backend nominal para `seção eleitoral` e `local de votação`.

## Atualização técnica (2026-03-08) - Conector nominal reorientado para `votacao_secao`

1. Backend de ingestão:
   - `src/pipelines/tse_candidate_votes.py` deixou de usar `tse_votacao_candidato_munzona` e passou a apontar para `tse_votacao_secao`;
   - o parse nominal agora captura também:
     - `NR_LOCAL_VOTACAO`
     - `NM_LOCAL_VOTACAO`
     - `DS_LOCAL_VOTACAO_ENDERECO`
2. Dimensão territorial:
   ...
   - a `dim_territory.metadata` fica preparada para derivar o `local de votação` com mais consistência.
3. Estado operacional após a mudança:
   - a mudança já está implementada em código e validada em testes unitários;
   - o ambiente ainda precisa de backfill histórico para substituir a base nominal hoje carregada por `zona eleitoral`.
4. Validação:
...
## Atualização técnica (2026-03-07) - Ranking eleitoral por distrito e auditoria pós-carga nominal

1. Frontend executivo:
   - o ranking de locais de votação passou a ser agrupado por distrito;
   - `seções` agora aparecem como contagem + prévia curta, em vez de lista extensa na mesma linha;
   - a coluna `Indicador selecionado` deixa de aparecer quando a métrica do ranking é `voters`, evitando duplicidade com `Eleitores`.
2. Estabilidade:
   - a regressão de hooks introduzida no agrupamento por distrito foi corrigida;
   - `ElectorateExecutivePage.test.tsx` voltou a passar integralmente (`5 passed`).
3. Auditoria atualizada:
   - `data/reports/electorate_consistency_audit.json` foi regenerado após a carga nominal de `2024`;
   - `summary`, `history`, `composição` e ranking de locais de votação seguem batendo entre API, Silver e bruto TSE.
4. Diagnóstico operacional atual:
   - as tabelas nominais (`silver.dim_election`, `silver.dim_candidate`, `silver.fact_candidate_vote`) já existem e estão carregadas neste ambiente;
   - o endpoint `GET /v1/electorate/election-context` agora resolve o contexto municipal com `source_level=electoral_zone` quando essa é a melhor granularidade disponível;
   - a limitação remanescente ficou em `GET /v1/electorate/candidate-territories`, que ainda depende de base nominal em `local de votação` ou `seção eleitoral`.
5. Próximo passo executável real:
   - adaptar a camada nominal para expor, no backend/frontend, quando a distribuição por candidato só existe em `zona eleitoral`;
   - só descer para `local de votação`/`seção eleitoral` quando houver fonte nominal compatível.

## Atualização técnica (2026-03-07) - UX-1 slice 3 frontend e auditoria do eleitorado

1. Frontend executivo:
   - `ElectorateExecutivePage` agora consome `GET /v1/electorate/election-context` e `GET /v1/electorate/candidate-territories`;
   - a tela passou a mostrar contexto da eleição, top candidatos e distribuição territorial do candidato selecionado;
   - a interação continua subordinada à leitura territorial, sem virar painel de apuração genérico.
2. Robustez backend:
   - `src/app/api/routes_qg.py` passou a degradar com segurança quando as tabelas nominais ainda não existem neste ambiente;
   - nesse cenário a API devolve `notes=candidate_tables_missing` e lista vazia, em vez de erro 500.
3. Auditoria executada:
   - novo script `scripts/audit_electorate_consistency.py` gerou `data/reports/electorate_consistency_audit.json`;
   - `summary`, `history` e `composição` do eleitorado bateram entre API, Silver e bruto TSE para `2016`, `2018`, `2020`, `2022` e `2024`;
   - o ranking de locais de votação `2024` bateu integralmente entre API e Silver;
   - o bruto `perfil_eleitorado` não expõe local de votação, então essa parte da auditoria depende da agregação Silver por seção/local.
4. Diagnóstico operacional do ambiente:
   - este banco local ainda não possui `silver.fact_candidate_vote`, `silver.dim_candidate` e `silver.dim_election`;
   - por isso o slice nominal do eleitorado já está exposto no frontend, mas permanece vazio até a migração/carga nominal ser aplicada.
5. Próximo passo executável real:
   - aplicar a migração/carga nominal do eleitorado neste ambiente;
   - rerodar a auditoria para incluir `election-context` e `candidate-territories` com dados reais;
   - só depois propagar essa leitura para `Home` e `Prioridades`.

## Atualização tecnica (2026-03-07) - UX-1 slice 3 backend nominal

1. Modelagem Silver entregue:
   - `silver.dim_election`;
   - `silver.dim_candidate`;
   - `silver.fact_candidate_vote`.
2. Ingestao:
   - novo pipeline `src/pipelines/tse_candidate_votes.py`;
   - Bronze dedicado para `tse_votacao_candidato_munzona`;
   - upsert idempotente de eleicao, candidato e voto territorial por candidato.
3. API executiva:
   - `GET /v1/electorate/election-context`;
   - `GET /v1/electorate/candidate-territories` com `aggregate_by=polling_place|electoral_section`.
4. Integracao operacional:
   - `configs/connectors.yml`, `configs/jobs.yml`, `configs/waves.yml` e `configs/schema_contracts.yml` atualizados com `tse_candidate_votes_fetch`;
   - `src/orchestration/prefect_flows.py`, `scripts/run_incremental_backfill.py`, `scripts/backfill_robust_database.py` e `scripts/equalize_database_env.ps1` passaram a incluir o novo job.
5. Evidencias:
...
## Atualização técnica (2026-03-07) - UX-1 slice 2 no mapa executivo

1. Frontend:
   - `frontend/src/modules/qg/pages/QgMapPage.tsx` passou a expor seletor de métrica eleitoral quando o overlay de locais de votação está ativo;
   - o mapa agora suporta alternância entre:
     - `voters`
     - `turnout`
     - `abstention_rate`
     - `blank_rate`
     - `null_rate`
   - tooltip, ranking e drawer foram sincronizados para refletir a mesma métrica eleitoral por local.
2. Contrato consumido:
   - `GET /v1/electorate/map` com `aggregate_by=polling_place` para geometria;
   - `GET /v1/electorate/polling-places` para ranking executivo, share municipal, distrito, zonas e seções.
3. Comportamento entregue:
   - ranking inferior do mapa deixa de mostrar tabela municipal quando o overlay eleitoral está ativo;
   - a seleção de uma linha do ranking foca o mesmo `territory_id` no mapa;
   - o drawer do local exibe eleitores base, share municipal, distrito, zonas e seções no mesmo contexto cartográfico.
4. Evidências:
   - `npm --prefix frontend run test -- --run src/modules/qg/pages/QgPages.test.tsx` -> `24 passed`;
   - `npm --prefix frontend run build` -> `OK`.

## Atualização técnica (2026-03-07) - UX-1 slice 1 no eleitorado

1. Backend:
   - `src/app/api/routes_qg.py` recebeu:
     - `GET /v1/electorate/history`
     - `GET /v1/electorate/polling-places`
   - `src/app/schemas/qg.py` passou a expor models dedicados para histórico anual e ranking de locais.
   - correção aplicada para evitar `comparecimento` acima de `total_voters`:
     - agregação eleitoral agora fixa um único `office`/`election_round` por ano (maior turnout) antes de calcular taxas.
2. Frontend:
   - `frontend/src/modules/electorate/pages/ElectorateExecutivePage.tsx` foi reestruturada para trocar a tabela municipal simples por:
     - histórico eleitoral anual;
     - ranking executivo de locais de votação;
     - fallback de ano preservado para ambos os blocos.
3. Testes:
   - `tests/unit/test_qg_routes.py` cobriu histórico eleitoral e ranking de locais por eleitores/comportamento.
   - `frontend/src/modules/electorate/pages/ElectorateExecutivePage.test.tsx` foi atualizado para validar:
     - filtros;
     - fallback com último ano disponível;
     - tolerância a falhas de fallback quando o ano filtrado já tem dados;
...
5. Observação operacional:
   - o teste Vitest precisou rodar fora da sandbox por `EPERM` local no acesso a `C:\\Users\\vtham`, sem indício de falha do código da nova tela.

## Atualização documental (2026-03-07) - Direção da expansão eleitoral

1. Decisão tomada:
   - a expansão de eleitorado não seguirá para uma tela de apuração genérica;
   - seguirá para leitura territorial defensável orientada ao mapa.
2. Próxima camada recomendada de evolução:
   - tipo da eleição;
   - turno;
   - cargo principal do ano;
   - candidatos relevantes;
   - votação por candidato em município, zona, local de votação e seção;
   - margem entre primeiro e segundo;
   - concentração e fragmentação territorial do voto.
3. Modelo recomendado para suportar essa expansão com clareza:
   - `silver.dim_election`;
   - `silver.dim_candidate`;
   - `silver.fact_candidate_vote`.
4. Regra de produto:
   - candidato entra como contexto analítico do território;
   - o território continua sendo a unidade principal de leitura;
   - o primeiro recorte deve privilegiar o cargo principal do ano para evitar mistura indevida de cargos no resumo executivo.

## Atualização documental (2026-03-06) - Ajuste do north star do mapa

1. `docs/VISION.md` refinado para incorporar a linha atual do produto:
   - distrito e setor censitário condicionados a densidade analítica real;
   - distinção explícita entre camada oficial, proxy e híbrida;
   - progressão do mapa por zoom e granularidade;
   - base urbana tratada como parte do valor analítico do produto.
2. Efeito esperado:
   - reduzir ambiguidade entre visão de produto e spec de mapa;
   - impedir exposição precoce de recortes territoriais sem narrativa útil;
   - manter o mapa alinhado ao objetivo de leitura executiva defensável.
3. Próximo impacto prático:
   - usar esse refinamento como referência para futuras decisões de UX do mapa e de ativação de camadas intraurbanas.

## Atualização documental (2026-03-06) - Diagnóstico das telas e eixo eleitoral

1. Eleitorado:
   - o backend já suporta resumo executivo e mapa por local de votação com fallback temporal;
   - a tela `ElectorateExecutivePage` ainda está excessivamente municipal e pouco territorial;
   - falta ranking executivo de locais, leitura histórica e comportamento eleitoral por local além de volume.
2. Mapa QG:
   - o mapa já está correto ao usar `local de votação` como unidade principal;
   - ainda precisa de lentes mais orientadas e leitura mais forte de acesso, cobertura e prioridade.
3. Home executiva:
   - já cumpre papel de síntese;
   - ainda depende excessivamente de listas e cards, sem converter o mapa em centro claro da leitura.
4. Prioridades:
   - estrutura funcional está boa;
   - precisa reforçar vínculos territoriais, comparações e transição para ação no mapa.
5. Insights:
   - entrega narrativa curta;
   - ainda precisa de trilha mais forte entre insight, território afetado, evidência espacial e recomendação operacional.
6. Cenários:
   - fluxo atual é funcional, mas simplificado e restrito a município;
   - ainda não sustenta simulação territorial mais rica nem leitura espacial do impacto.
7. Briefs:
   - já exporta HTML/PDF de forma útil;
   - precisa incorporar melhor contexto territorial e recorte espacial prioritário.
8. Perfil territorial:
   - bom como tela analítica;
   - ainda precisa aproximar mais a comparação territorial da lógica executiva do mapa.
9. Admin/Ops:
   - conjunto está funcional e coerente como camada técnica;
   - não demanda expansão visual prioritária neste ciclo.
10. Diretriz resultante:
   - o maior gap atual de frontend não é infraestrutura;
   - é transformar as telas em narrativa executiva territorial coesa, com eleitorado e mapa operando como eixo principal de defesa do produto.
11. Sequência única aprovada para a próxima frente funcional:
...
12. Próximo passo executável quando a frente funcional for reaberta:
   - iniciar `UX-1`, com foco em ranking de locais de votação, histórico eleitoral e comportamento por local.

## Atualização tecnica (2026-03-03) - Hotfix de conectividade da UI (CORS)
...
## Atualização tecnica (2026-03-03) - Fechamento de pendencias D7 e estabilizacao da suite QG
...
2. Banco:
   - `db/sql/019_strategic_score_weights_audit.sql` adicionado com:
     ...
     - registro de alterações de pesos/configuracao com `changed_fields`, `weights_changed`, `old_row` e `new_row`.
3. Frontend testes:
...
## Atualização tecnica (2026-02-27) - Hotfix Home/Cenarios
...
## Atualização tecnica (2026-02-26) - Refatoracao executiva QG + dominio Portal Transparencia

1. Backend/API (coerencia de dominio):
   - `src/app/api/routes_qg.py` e `db/sql/015_priority_drivers_mart.sql` alinhados para mapear:
     - `PORTAL_TRANSPARENCIA/assistencia_social` -> `assistencia_social`;
     - `PORTAL_TRANSPARENCIA/financas_publicas` -> `financas`;
     - `SUASWEB` e `CNEAS` -> `assistencia_social`.
   - normalizacao defensiva adicionada no payload de prioridades e KPIs para evitar divergencia de taxonomia no frontend.
2. Frontend (valor estrategico das telas):
   - Home (`QgOverviewPage`) refatorada para leitura executiva sem duplicar o mapa da aba dedicada;
   - Prioridades (`QgPrioritiesPage`) com cards-resumo por severidade;
   - Insights (`QgInsightsPage`) agrupado por severidade e com CTA para mapa/brief;
   - Cenarios (`QgScenariosPage`) com resultado objetivo + detalhes colapsaveis;
   - Eleitorado (`ElectorateExecutivePage`) com CTA para mapa eleitoral e composicao em abas.
3. Catalogo de dominio:
   - `frontend/src/modules/qg/domainCatalog.ts` ampliado com `assistencia_social` e `geral`.
...
5. Risco pendente conhecido:
   - suite completa de frontend ainda tem descompasso de contrato em `frontend/src/modules/qg/pages/QgPages.test.tsx` após refatoracao de Home/Mapa.
6. Proximo passo imediato:
   - executar rodada dedicada de estabilizacao de `frontend/src/modules/qg/pages/QgPages.test.tsx`, alinhando expectativas ao layout atual de `QgOverviewPage` e `QgMapPage`.

## Atualização operacional (2026-02-26) - Guardrail de geolocalizacao eleitoral por distrito

1. Correção estrutural no fluxo de geolocalização de locais de votação:
   - `scripts/build_seed.py` agora suporta `data/seed/polling_places_overrides_diamantina.csv` para correções controladas;
   - overrides só são aplicados quando latitude/longitude estiverem preenchidas.
2. Bloqueio de coordenada inconsistente no apply:
   - `scripts/apply_seed.py` passou a validar `expected_district` (quando informado no CSV de overrides);
   - ponto fora do distrito esperado é `SKIP` (não persiste no banco).
3. Auditoria geoespacial endurecida:
   - `scripts/audit_polling_places_geolocation.py` agora detecta:
     - pontos fora de qualquer distrito (`outside_any_district`);
     - pontos fora do distrito esperado (`outside_expected_district`);
     - violações de distância para coordenadas de override (`override_distance_violations`).
4. Governança inicial do arquivo de overrides:
   - `data/seed/polling_places_overrides_diamantina.csv` criado com regra de distrito esperado para `1252` e `1341` (sem forçar coordenada não homologada).
5. Evidências:
   - `.\.venv\Scripts\python.exe scripts/apply_seed.py` -> `Total sections updated: 144`, `Skipped by district rule: 0`;
   - `.\.venv\Scripts\python.exe scripts/audit_polling_places_geolocation.py` -> `status=pass`;
   - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_qg_routes.py tests/unit/test_tse_electorate.py -q` -> `37 passed`.
6. Próximo passo imediato:
   - preencher coordenadas no CSV de overrides apenas para locais já homologados em campo/equipe local;
   - reaplicar `build_seed -> apply_seed -> audit` para manter correção distrital sem regressão.

## Atualização operacional (2026-02-26) - Revisao de pontos suspeitos em distritos (imagem de homologacao)
...
2. Resultado da verificação:
   ...
   - correção aplicada em `1341`, que foi deslocado para o núcleo de Conselheiro Mata com
     coordenada `(-18.287522, -43.981900)`.
3. Governança de consistência:
   - `data/seed/polling_places_overrides_diamantina.csv` expandido com regras de `expected_district`
     para os locais revisados.
4. Robustez adicional no ciclo de ambiente:
   - `scripts/equalize_database_env.ps1` agora inclui `build_seed` + `audit_polling_places_geolocation`;
   - a equalização passa a falhar automaticamente se o audit de geolocalização eleitoral não estiver em `pass`.
5. Evidências:
   - `.\.venv\Scripts\python.exe scripts/build_seed.py` -> `Overrides applied: 1`;
   - `.\.venv\Scripts\python.exe scripts/apply_seed.py` -> `Total sections updated: 144`;
   - `.\.venv\Scripts\python.exe scripts/audit_polling_places_geolocation.py` -> `status=pass`.

## Atualização operacional (2026-02-26) - Busca web complementar e ajuste de povoados sem geocode preciso

1. Busca na internet executada para locais com suspeita de deslocamento:
   - `Mão Torta`, `Batatal`, `Baixadão` e `Capoeirão` (Rogério Firmino Lopes).
2. Ajustes aplicados no override:
   - `1406` (`Baixadão`) reposicionado para `Planalto de Minas` com coordenada de geocode público;
   - `1457` (`Batatal`) reposicionado para área de `Conselheiro Mata` com proxy de assentamento;
   - `1414` (`Rogério Firmino Lopes`) e `1422` (`Mão Torta`) movidos para proxies distritais de `Desembargador Otoni` por ausência de ponto público preciso.
3. Regra operacional reforçada:
   - executar sempre em sequência `build_seed -> apply_seed -> audit`;
   - não rodar `apply_seed` e `audit` em paralelo (risco de ler estado antigo).
4. Evidências da rodada:
   - `.\.venv\Scripts\python.exe scripts/build_seed.py` -> `Overrides loaded: 5`;
   - `.\.venv\Scripts\python.exe scripts/apply_seed.py` -> `Total sections updated: 144`;
   - `.\.venv\Scripts\python.exe scripts/audit_polling_places_geolocation.py` -> `status=pass`.

## Atualização operacional (2026-02-26) - Coordenadas homologadas pelo usuario (Google)

1. Coordenadas aplicadas no override:
...
2. Pipeline executado em sequência:
   - `build_seed -> apply_seed -> audit_polling_places_geolocation`.
3. Resultado:
   - atualização aplicada sem bloqueio de distrito (`Skipped by district rule: 0`);
   - auditoria final em `pass`.

## Atualização operacional (2026-02-26) - Seções de locais de votação no tooltip
...
## Atualização operacional (2026-02-25) - Validacao geoespacial completa + acentuacao transversal
...
## Atualização operacional (2026-02-25) - Refino geoespacial de locais de votação e acentuação transversal de UI
...
## Atualização operacional (2026-02-25) - Correção de visualização de locais de votação no mapa
...
## Atualização operacional (2026-02-25) - Equalizacao unificada de banco entre ambientes

1. Script único de equalizacao adicionado:
...
## Atualização operacional (2026-02-25) - Contrato de URL do mapa sem métrica legada
...
## Atualização operacional (2026-02-25) - Varredura backend de tiles (`/v1/map/tiles/*`)
...
## Atualização operacional (2026-02-25) - Varredura frontend de tiles e bootstrap da rota `/mapa`
...
## Atualização operacional (2026-02-25) - Exibição de locais de votação no mapa (fallback de ano)
...
## Atualização operacional (2026-02-25) - OSM-only + agregação eleitoral por local de votação
...
## Atualização operacional (2026-02-25) - RCA + hotfix locais de votação sem retorno
...
## Atualização operacional (2026-02-25) - Correção overlays UBS/Escola + sidebar de camadas
...
## Atualização operacional (2026-02-25) - Reestruturação completa do mapa estratégico
...
## Atualização operacional (2026-02-24) - Opção (a) concluída com zeragem de warnings
...
## Atualização operacional (2026-02-24) - Mapa estratégico multicamadas (foco executivo)
...
6. Evidência técnica da rodada:
  ...
  - `npm --prefix frontend run build` -> `OK`;
  - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_qg_routes.py tests/unit/test_tse_electorate.py -q` -> `37 passed`;
  - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_mvt_tiles.py tests/unit/test_cache_middleware.py -q` -> `30 passed`.
6. Estado atual:
...
## Atualização operacional (2026-02-24) - Backfill temporal ANA/INMET/INPE concluído
...
## Atualização operacional (2026-02-24) - Conector Portal da Transparência municipal
...
## Atualização operacional (2026-02-24) - Desbloqueio SENATRAN 2024 e fechamento de gate
...
## Atualização operacional (2026-02-24) - Rodada focal INEP/SICONFI/TSE (resultado)
...
## Atualização operacional (2026-02-24) - Calibragem de checks por disponibilidade real (opcao b)
...
## Atualização operacional (2026-02-23) - Ícones SVG, renomeação e fix drawer
...
## Atualização operacional (2026-02-23) - Executive Design System v2
...
## Atualização operacional (2026-02-23) - Correcoes de utilidade estratégica no mapa (feedback visual)
...
1. Correção aplicada para aumentar valor decisório do mapa executivo:
  ...
  - bloco operacional `Top seções por eleitorado` exibindo ranking de seções com maior volume de eleitores.
...
## Atualização operacional (2026-02-23) - Homologacao backend do evento operacional do mapa
...
## Atualização operacional (2026-02-23) - Sprint P0 mapa (telemetria de estado operacional)
...
## Atualização operacional (2026-02-23) - Sprint P0 mapa executivo (previsibilidade de estados)
...
## Atualização operacional (2026-02-23) - Hardening de robustez frontend (Admin + observabilidade)
...
## Atualização operacional (2026-02-24) - Telemetria objetiva da troca de camada eleitoral
...
## Atualização operacional (2026-02-23) - UX final do mapa executivo (drawer territorial)

1. Implementacao de UX do mapa em ciclo curto concluida no frontend:
  - `frontend/src/modules/qg/pages/QgMapPage.tsx` migrado para painel territorial em drawer com estrutura executiva (status, tendência, valor, métricas, evidências e ações);
...
## Atualização operacional (2026-02-23) - Fechamento local_votacao no mapa executivo
...
## Atualização operacional (2026-02-23) - Legenda visual eleitoral e navegação lateral
...
## Atualização operacional (2026-02-23) - Refatoracao completa de design do frontend
...
## Atualização operacional (2026-02-23) - Backend/DB fechado para foco em frontend
...
## Atualização operacional (2026-02-23) - Fechamento backend/db (delta de confiabilidade)
...
## Atualização operacional (2026-02-23) - Integracao aberta SUASWEB/CNEAS consolidada
...
## Atualização operacional (2026-02-23) - Foco ativo em dados abertos (sem bloqueio por credencial)
...
## Atualização operacional (2026-02-23) - Lacunas de dados executáveis tratadas
...
## Atualização operacional (2026-02-23) - Secao eleitoral no banco/API (Diamantina/MG)
...
## Atualização operacional (2026-02-23) - Camadas eleitorais sem pendencia de geometria
...
## Atualização operacional (2026-02-23) - Fechamento de qualidade do eleitorado (Diamantina)
...
## Atualização operacional (2026-02-23) - Backfill histórico 2022 concluído
...
## Atualização operacional (2026-02-23) - Backfill histórico 2020 concluído
...
## Atualização operacional (2026-02-23) - Backfill histórico 2018 concluído + robustez NaN
...
## Atualização operacional (2026-02-23) - Ingestao TSE executada para zona/seção eleitoral
...
## Atualização tecnica (2026-02-23) - local_votacao consolidado no mapa executivo

1. Implementacao frontend concluida em `frontend/src/modules/qg/pages/QgMapPage.tsx`:
  - toggle rápido para alternar `Locais de votacao` <-> `Secoes eleitorais` no nivel `secao_eleitoral`;
...
2. Cobertura de regressao ampliada em `frontend/src/modules/qg/pages/QgPages.test.tsx`:
  ...
  - validacao de botao `Exibir seções eleitorais` quando `local_votacao` esta ativo.
...
## Atualização tecnica (2026-02-23) - Refino de legibilidade operacional no mapa
...
## Atualização tecnica (2026-02-23) - Origem da camada explicita no painel do mapa
...
## Atualização tecnica (2026-02-23) - Aviso explicito de reset do foco territorial
...
## Atualização tecnica (2026-02-23) - Aviso de recentralizacao automatica no mapa
...
## Atualização tecnica (2026-02-23) - Utilidade executiva imediata no mapa
...
## Atualização operacional (2026-02-23) - Homologacao recorrente do mapa concluida
...
## Atualização tecnica (2026-02-22) - Homologacao operacional do mapa concluida
...
## Atualização tecnica (2026-02-22) - P1 telemetria do mapa operacionalizada
...
## Atualização tecnica (2026-02-22) - Fechamento da rodada P0
...
## Atualização tecnica (2026-02-22) - P0 ciclo completo no Eleitorado
...
## Atualização tecnica (2026-02-22) - P0 continuidade em Território 360
...
## Atualização tecnica (2026-02-22) - P0 iniciado no mapa executivo
...
## Atualização tecnica (2026-02-22) - Drift operacional no histórico de robustez
...
## Atualização operacional (2026-02-22) - Monitoramento recorrente (estabilidade mantida)
...
2. Persistencia de histórico atualizada:
...
## Atualização operacional (2026-02-22) - Rodada de consolidacao 30d e gates tecnicos
...
2. Histórico de robustez consultado em `GET /v1/ops/robustness-history?page_size=5`:
  - `total=2` snapshots;
...
3. Gates tecnicos da rodada executados:
  ...
  - `\.\.venv\Scripts\python.exe -m pytest tests/unit/test_mvt_tiles.py tests/unit/test_cache_middleware.py -q -p no:cacheprovider` -> `30 passed`.
...
## Atualização operacional (2026-02-22) - Recuperacao de SLO-1 concluida (warnings=0)
...
## Atualização técnica (2026-02-22) - Janela 30d em READY com gates consolidados
...
## Atualização técnica (2026-02-22) - Consolidação operacional 30d publicada (pós-D8)
...
4. Validação executada:
   - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_ops_robustness_window.py tests/unit/test_ops_routes.py -q -p no:cacheprovider` -> `30 passed`.
...
## Atualização técnica (2026-02-22) - D8 BD-082 implementado (playbook de incidentes e operação assistida)

1. Snapshot operacional único para triagem de incidente:
...
5. Governança de issue na trilha unica:
   - `BD-082` concluído tecnicamente.
   - trilha D8 encerrada tecnicamente; próximo passo e consolidação operacional (janela de 30 dias).

## Atualização técnica (2026-02-22) - D8 BD-081 implementado (tuning de performance e custo da plataforma)
...
## Atualização técnica (2026-02-22) - D8 BD-080 implementado (carga incremental confiavel + reprocessamento seletivo)
...
3. Entrega esperada para demonstracao:
   - valor visível no mapa com prioridade explicavel por território;
   - fluxo estavel em estados `loading/error/empty/data`;
   - evidências técnicas registradas em `CHANGELOG` e neste `HANDOFF`.

## Atualização técnica (2026-02-22) - D7 BD-072 implementado (trilhas de explicabilidade para prioridade/insight)
...
7. Governança de issue na trilha unica:
   - `BD-072` concluído tecnicamente.
   - próximo item da fila unica: `D8/BD-080`.

## Atualização técnica (2026-02-22) - D7 BD-071 implementado (versionamento de score territorial e pesos)
...
4. API executiva e contratos:
   - `src/app/api/routes_qg.py` passa a expor metadados/evidências com `score_version`, `scoring_method` e pesos;
...
6. Validação executada:
   ...
   - `.\.venv\Scripts\python.exe scripts/export_data_coverage_scorecard.py --output-json data/reports/data_coverage_scorecard.json` -> `pass=28`, `warn=4`.
7. Governança de issue na trilha unica:
   - `BD-071` concluído tecnicamente.
   - sequencia da fila unica avancou para `BD-072` (concluído na atualizacao superior deste documento).

## Atualização técnica (2026-02-22) - D7 BD-070 implementado (mart Gold de drivers de prioridade)
...
## Atualização técnica (2026-02-22) - D6 BD-062 implementado (detectar drift de schema com alerta operacional)
...
## Atualização técnica (2026-02-22) - D6 BD-061 implementado (cobertura de testes de contrato por conector)
...
## Atualização técnica (2026-02-22) - D6 BD-060 implementado (contratos de schema versionados por fonte)
...
4. Validação executada:
   - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_quality_core_checks.py tests/unit/test_quality_suite.py tests/unit/test_schema_contracts.py tests/contracts/test_sql_contracts.py -q -p no:cacheprovider` -> `30 passed`.
   - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_quality_suite.py tests/unit/test_quality_core_checks.py tests/unit/test_quality_coverage_checks.py tests/unit/test_quality_ops_pipeline_runs.py -q -p no:cacheprovider` -> `23 passed`.
...
## Atualização técnica (2026-02-21) - D5 BD-052 implementado (mart Gold de risco ambiental territorial)
...
## Atualização técnica (2026-02-21) - D5 BD-051 implementado (agregações ambientais distrito/setor)
...
## Atualização técnica (2026-02-21) - D5 BD-050 implementado (histórico ambiental multi-ano)

1. Orquestracao dedicada para BD-050 publicada:
   - novo script `scripts/backfill_environment_history.py` com fluxo único de:
...
## Atualização técnica (2026-02-21) - D4 BD-042 implementação de mart de mobilidade
...
## Atualização técnica (2026-02-21) - D4 BD-041 implementação de transporte urbano
...
## Atualização operacional (2026-02-21) - BD-040 executado com backfill real

1. Bootstrap histórico SENATRAN concluído:
   ...
   - evidências em `data/reports/bootstrap_senatran_history_report.json`.
...
## Atualização técnica (2026-02-21) - D4 BD-040 hardening SENATRAN
...
4. Validação executada:
   - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_db_cache.py tests/unit/test_onda_a_connectors.py tests/unit/test_quality_coverage_checks.py -q -p no:cacheprovider` -> `30 passed`.
   - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_qg_routes.py tests/unit/test_mvt_tiles.py tests/unit/test_cache_middleware.py -q -p no:cacheprovider` -> `44 passed`.
...
## Atualização operacional (2026-02-20) - Filtro rigoroso de documentos
...
## Atualização técnica (2026-02-20) — Fase UX-P0 entregue
...
## Atualização técnica (2026-02-20) — Fase DATA entregue
...
3. Arquivos modificados:
   - `src/app/api/routes_qg.py` (backend — 6 alterações + 4 funções novas).
...
## Atualização de planejamento (2026-02-20) - Backlog UX executivo unificado

1. Backlog único consolidado para correções de layout/legibilidade:
...
4. Mapeamento de escopo:
   - backlog já inclui arquivos/componentes alvo por página (`Prioridades`, `Mapa`, `Territorio 360`, `Insights`, `Cenarios`, `Briefs`, `Eleitorado`, `App shell`).

## Atualização operacional (2026-02-20) - Governança de issues GitHub

1. Trilha ativa oficial no GitHub:
   - `BD-033` criada em `#28` com label `status:active`.
2. Fechamento de item concluído:
...
## Atualização operacional (2026-02-20) - Fechamento de gate BD-033 + fase 2

1. Gate técnico da trilha ativa revalidado:
   - `.\.venv\Scripts\python.exe -m pytest tests/unit/test_qg_routes.py tests/unit/test_tse_electorate.py -q` -> `30 passed`.
...
## Atualização técnica (2026-02-20) - Hotfix UX mapa (legibilidade + area util)
...
1. Correções de legibilidade dos controles do mapa:
   ...
   - impacto: botoes não selecionados deixaram de ficar visualmente "invisíveis".
...
## Atualização técnica (2026-02-20) - Painel de filtros do mapa situacional (layout e formatacao)
...
## Atualização técnica (2026-02-20) - Transparencia de classificação de camadas (mapa)
...
- `frontend/src/modules/qg/pages/QgMapPage.tsx`:
  ...
  - tooltip da camada ativa passou a priorizar metodologia (`proxy_method`) para leitura rápida de limitações.
...
## Atualização técnica (2026-02-20) - Quality Suite (ativacao de checks de camadas do mapa)
...
## Atualização técnica (2026-02-20) - Home QG (degradação parcial de prioridades/destaques)
...
- `frontend/src/modules/qg/pages/QgOverviewPage.tsx`:
  ...
  - objetivo: preservar navegacao do mapa, situacao geral e ações rápidas mesmo com falha parcial de dados secundarios.
- `frontend/src/modules/qg/pages/QgPages.test.tsx`:
...
## Atualização técnica (2026-02-20) - Mapa executivo (estados de suporte padronizados)
...
## Atualização técnica (2026-02-20) - Boundary de erro por rota (frontend)
...
- `frontend/src/app/router.tsx`:
  ...
  - objetivo: evitar tela branca em erro de render e manter navegacao operacional previsível.
- `frontend/src/app/RouteRuntimeErrorBoundary.test.tsx`:
  - novo teste cobrindo:
    - exibição de estado de erro e emissao de telemetria em crash de render;
    - recuperacao da tela após `Tentar novamente`.
- Validação executada:
...
## Atualização técnica (2026-02-20) - Ops Health (refresh + regressão readiness)
...
## Atualização técnica (2026-02-20) - Home QG (camada detalhada coerente)
...
## Atualização técnica (2026-02-20) - Estabilizacao mapa + eleitorado
...
- Backend:
  - `pytest tests/unit/test_qg_routes.py tests/unit/test_tse_electorate.py -q` -> `30 passed`.
...
## Atualização técnica (2026-02-20) - Mapa semantico (sem dado)
...
## Atualização técnica (2026-02-20) - Mapa com rotulos e zoom contextual
...
- Diretriz oficial de foco publicada e consolidada em `docs/BACKLOG_DADOS_NIVEL_MAXIMO.md` (seções 7, 8 e 9).
- Ordem de execução obrigatoria no ciclo atual:
  ...
  - 2) fechar gates de confiabilidade (qualidade/readiness/smokes e evidências operacionais);
...
## Atualização técnica (2026-02-19) - QG Prioridades (paginacao)
...
- `frontend/src/modules/qg/pages/QgPrioritiesPage.tsx`:
  ...
  - resumo da lista agora evidencia `visíveis`, `filtradas` e `retorno bruto`.
- `frontend/src/modules/qg/pages/QgPages.test.tsx`:
...
## Atualização técnica (2026-02-19) - Mapa vetorial (controles de navegacao)
...
## Atualização técnica (2026-02-19) - Estabilizacao de telas criticas
...
## Atualização técnica (2026-02-18) - Robustez de banco

- Hardening de cobertura territorial concluído no backend:
...
- Sprint D0 da trilha de robustez maxima concluído:
...
- Sprint D3 avancado para ingestao e geocodificacao local (2026-02-19):
  ...
  - monitor técnico de camadas atualizado no frontend Ops:
    ...
    - resumo operacional adicional publicado na tela:
      ...
      - grade de "Resumo rápido das camadas" por item com chips de `rows`, `geom` e `readiness`.
  ...
  - `BD-033` avancado (iteracao atual) com deep-link completo:
    ...
    - navegacao territorial ampliada para aproximar UX de mapa operacional:
      - busca rápida de território no `QgMapPage` (`Buscar territorio` + `Focar territorio`).
    ...
    - ações contextuais urbanas publicadas no card de seleção:
      - filtro rápido por classe/categoria (`/v1/map/urban/roads` e `/v1/map/urban/pois`).
    ...
    - `QgOverviewPage` evoluida para `Layout B` (mapa dominante):
      ...
      - painel lateral colapsavel com filtros principais, cards de status e ações rápidas.
...
4. Specs estrategicas promovidas a v1.0 com fases concluidas marcadas:
   - `MAP_PLATFORM_SPEC.md` (MP-1, MP-2 e MP-3 baseline concluídos)
   - `TERRITORIAL_LAYERS_SPEC_DIAMANTINA.md` (TL-1, TL-2 e TL-3 baseline concluídos)
   - `STRATEGIC_ENGINE_SPEC.md` (SE-1 e SE-2 concluídos)
...
## Atualização técnica (2026-02-13)
...
- **Validações da iteracao**:
  ...
  - frontend: `QgPages.test.tsx` passando após incluir seletor de camada; build Vite passando.
...
- Runbook de operações (`OPERATIONS_RUNBOOK.md`):
  - 12 seções cobrindo todo ciclo de vida: ambiente, pipelines, qualidade, views, API, frontend, go-live, testes, troubleshooting, conectores especiais, deploy (11 passos + rollback).
- Specs v0.1 → v1.0:
...
- Script de homologação consolidado (`scripts/homologation_check.py`):
  ...
  - Verdict único READY/NOT READY com suporte `--json` e `--strict`.
...
- Extensao v1.1 iniciada no backend:
  ...
  - `POST /v1/briefs` para geracao de brief executivo com resumo e evidências.
...
- Sprint 6 técnico (Onda B/C) iniciado no backend:
  ...
  - `ANEEL` foi ajustado para `prefer_manual_first` no conector, reduzindo custo de execução
    local quando o CSV municipal consolidado já existe em `data/manual/aneel`.
...
- Frontend integrado ao novo contrato QG:
  ...
  - Home QG passou a exibir `Top prioridades` (pré-visualização) e `Ações rápidas` para fluxo de decisão.
  ...
  - endpoint `GET /v1/territory/{id}/profile` evoluiu com score/status/tendência agregados do território:
    - `overall_score`
    - `overall_status`
    - `overall_trend`
  - `TerritoryProfilePage` passou a exibir card executivo de status geral com score consolidado e tendência.
  - endpoint `GET /v1/territory/{id}/peers` adicionado para sugerir comparações por similaridade de indicadores.
...
- Hardening frontend (Sprint 5) iniciado:
  - acessibilidade minima no shell: `skip link` para conteudo principal e foco visível padronizado.
...
- Validação frontend:
  ...
  - `npm --prefix frontend run typecheck` (após hardening de a11y/observabilidade): `OK`.
  - `npm --prefix frontend run typecheck` (após exportacao SVG/PNG): `OK`.
...
## Atualização operacional (2026-02-12)

- Filtros de domínio no fluxo QG padronizados no frontend:
  - `Prioridades`, `Insights`, `Briefs` e `Cenarios` agora usam `select` com catálogo único.
...
- Testes de regressão frontend reestabilizados após a evolução da Home:
...
- Validação executada em 2026-02-12 (ciclo atual):
  ...
  - `npm --prefix frontend run build`: `OK` (Vite build concluído com filtros padronizados + prefill por query string).
  - `npm --prefix frontend run test`: `14 passed` / `35 passed` (revalidado após rotulos amigaveis de domínio no QG).
  - `npm --prefix frontend run build`: `OK` (revalidado após refinamento de UX de domínio).
  - `npm --prefix frontend run test`: `14 passed` / `35 passed` (revalidado após padronizacao de rotulos no `TerritoryProfilePage`).
  - `npm --prefix frontend run build`: `OK` (revalidado após ajuste no `TerritoryProfilePage`).
...
## [HISTÓRICO] Próximos passos imediatos (após iteracao readiness API)

1. Expor `GET /v1/ops/readiness` tambem no painel técnico `/admin` como card de status único
   para triagem rápida de ambiente.
...
5. Para abertura/atualizacao rápida no GitHub:
...
- Suite unit completa atualizada após endpoints QG adicionais: `96 passed`.
...
- Validações recentes:
  ...
  - `npm run build` (frontend): build concluído (validado no terminal do usuario)
...
- Frontend:
  - F1 concluído no repositório (`frontend/`)
...
### Prioridade alta
1. Fechar estabilizacao de UX nas telas executivas (`/mapa`, `/territorio/:id`, `/eleitorado`) e registrar evidências de teste.
