...
JOB_NAME = "sejusp_public_safety_fetch"
SOURCE = "SEJUSP_MG"
DATASET_NAME = "sejusp_public_safety_catalog"
FACT_DATASET_NAME = "sejusp_public_safety_municipal"
WAVE = "MVP-4"
SEJUSP_CATALOG_PATH = Path("configs/sejusp_public_safety_catalog.yml")
MANUAL_SEJUSP_DIR = Path("data/manual/sejusp")
...
_TOTAL_VALUE_COLUMNS = ("ocorrencias_total", "crimes_total", "total_ocorrencias", "total")
_VIOLENT_COLUMNS = ("crimes_violentos", "crime_violento", "homicidio", "homicidios")
_ROBBERY_COLUMNS = ("roubo", "roubos")
_THEFT_COLUMNS = ("furto", "furtos")
...
def _load_catalog(path: Path = SEJUSP_CATALOG_PATH) -> list[dict[str, Any]]:
    if not path.exists():
        return []
    payload = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
    resources = payload.get("resources", [])
    if not isinstance(resources, list):
        raise ValueError("Invalid SEJUSP catalog format: 'resources' must be a list.")
...
def _list_manual_candidates(path: Path = MANUAL_SEJUSP_DIR) -> list[Path]:
    if not path.exists():
        return []
...
def _build_indicator_rows(
    *,
    ...
    indicators = [
        ("SEJUSP_OCORRENCIAS_TOTAL", "SEJUSP ocorrencias totais", total),
    ]

    violent = _first_numeric(municipality_row, _VIOLENT_COLUMNS)
    if violent is not None:
        indicators.append(("SEJUSP_CRIMES_VIOLENTOS", "SEJUSP crimes violentos", violent))

    robbery = _first_numeric(municipality_row, _ROBBERY_COLUMNS)
    if robbery is not None:
        indicators.append(("SEJUSP_ROUBOS", "SEJUSP roubos", robbery))

    theft = _first_numeric(municipality_row, _THEFT_COLUMNS)
    if theft is not None:
        indicators.append(("SEJUSP_FURTOS", "SEJUSP furtos", theft))
    ...
    *,
    ...
    for resource in resources:
        ...
        try:
            ...
            warnings.append(f"SEJUSP remote source failed for '{uri}': {exc}")
    ...
    for candidate in _list_manual_candidates():
        try:
            ...
            warnings.append(f"SEJUSP manual source failed for '{candidate.name}': {exc}")
...
def run(
    *,
    ...
    try:
        ...
        if dataset is None:
            warnings.append("No SEJUSP source available (remote catalog and manual directory failed).")
            ...
            if not dry_run:
                with session_scope(settings) as session:
                    ...
                    replace_pipeline_checks_from_dicts(
                        ...
                        checks=[
                            {
                                "name": "sejusp_source_resolved",
                                "status": "warn",
                                "details": "No SEJUSP source available (remote catalog and manual directory failed).",
                                "observed_value": 0,
                                "threshold_value": 1,
                            },
                            {
                                "name": "sejusp_indicator_rows_loaded",
                                "status": "warn",
        ...
        if municipality_row is None:
            warnings.append(
                f"SEJUSP row not found for municipality code/name ({municipality_ibge_code}/{municipality_name})."
            )
        ...
        checks = [
            ...
            {
                "name": "sejusp_municipality_row_found",
                "status": "pass" if municipality_row is not None else "warn",
                "details": "Municipality row should be present in SEJUSP dataset.",
        ...
        artifact = persist_raw_bytes(
            ...
            dataset_version="sejusp-public-safety-v1",
            checks=checks,
            notes="SEJUSP public safety extraction and indicator upsert.",
            run_id=run_id,
        ...
        elapsed = time.perf_counter() - started_at
        logger.info(
            "SEJUSP public safety job finished.",
            run_id=run_id,
            status=status,
            rows_extracted=rows_extracted,
            rows_written=rows_written,
            ...
            try:
                with session_scope(settings) as session:
                    ...
                    replace_pipeline_checks_from_dicts(
                        ...
                        checks=[
                            {
                                "name": "sejusp_job_exception",
                                "status": "fail",
                                "details": f"SEJUSP connector failed with exception: {exc}",
                                "observed_value": 1,
                                "threshold_value": 0,
                            }
                        ],
        ...
        logger.exception(
            "SEJUSP public safety job failed.",
            run_id=run_id,
            duration_seconds=round(elapsed, 2),
        )
        return {
