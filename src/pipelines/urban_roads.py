...
JOB_NAME = "urban_roads_fetch"
SOURCE = "OSM"
DATASET_NAME = "urban_roads_catalog"
WAVE = "MVP-7"
CATALOG_PATH = Path("configs/urban_roads_catalog.yml")
MANUAL_DIRS = (Path("data/manual/urban/roads"), Path("data/manual/urban"))
...
def _load_catalog(path: Path = CATALOG_PATH) -> list[dict[str, Any]]:
    ...
    if not isinstance(resources, list):
        raise ValueError("Invalid urban roads catalog format: 'resources' must be a list.")
    return [item for item in resources if isinstance(item, dict)]


def _resolve_municipality_context(
    settings: Settings,
...
def _extract_from_zip(raw_bytes: bytes) -> tuple[bytes, str]:
    with zipfile.ZipFile(io.BytesIO(raw_bytes)) as archive:
        ...
        if not names:
            raise ValueError("ZIP file has no supported road payload.")
...
def _parse_payload_bytes(raw_bytes: bytes, *, suffix: str) -> tuple[Any, bytes, str]:
    ...
    raise ValueError(f"Unsupported suffix for roads payload: {suffix}")


def _ensure_linestring_geometry(geometry: Any) -> dict[str, Any] | None:
    if not isinstance(geometry, dict):
        return None
    geometry_type = str(geometry.get("type", "")).strip()
    coordinates = geometry.get("coordinates")
    if geometry_type == "LineString" and isinstance(coordinates, list) and len(coordinates) >= 2:
        return {"type": "LineString", "coordinates": coordinates}
    if geometry_type == "MultiLineString" and isinstance(coordinates, list) and coordinates:
        longest = max(
            (item for item in coordinates if isinstance(item, list)),
...
def _parse_overpass_rows(payload: dict[str, Any], *, reference_period: str) -> list[dict[str, Any]]:
    ...
    for item in elements:
        ...
        rows.append(
            {
                "source": "OSM_OVERPASS",
                "external_id": f"way/{item.get('id')}",
                "name": str(tags.get("name") or "").strip() or None,
                "road_class": str(tags.get("highway") or "").strip() or None,
                "is_oneway": _to_bool(tags.get("oneway")),
                "metadata_json": {
...
def _parse_geojson_rows(payload: dict[str, Any], *, reference_period: str) -> list[dict[str, Any]]:
    ...
    for feature in features:
        if not isinstance(feature, dict):
            continue
        geometry = _ensure_linestring_geometry(feature.get("geometry"))
        if geometry is None:
            continue
        properties = (
        ...
        row_source = str(properties.get("source") or "MANUAL_URBAN").strip() or "MANUAL_URBAN"
        rows.append(
            {
                "source": row_source,
                "external_id": str(
                ...
                "road_class": str(
                    properties.get("road_class")
                    or properties.get("highway")
                    or properties.get("class")
                    or ""
                ).strip()
                or None,
                "is_oneway": _to_bool(properties.get("is_oneway") or properties.get("oneway")),
                "metadata_json": {
                    "reference_period": reference_period,
    ...
    for item in raw_rows:
        ...
        if geometry_json_raw:
            try:
                ...
                normalized_geometry = _ensure_linestring_geometry(geometry_payload)
        ...
        if geometry_json is None and geometry_wkt is None:
            start_lon = normalized.get("start_lon")
            start_lat = normalized.get("start_lat")
            end_lon = normalized.get("end_lon")
            end_lat = normalized.get("end_lat")
        ...
        row_source = str(normalized.get("source") or "MANUAL_URBAN").strip() or "MANUAL_URBAN"
        rows.append(
            {
                "source": row_source,
                ...
                "road_class": str(
                    normalized.get("road_class")
                    or normalized.get("highway")
                    or normalized.get("class")
                    or ""
                ).strip()
                or None,
                "is_oneway": _to_bool(normalized.get("is_oneway") or normalized.get("oneway")),
                "metadata_json": {
                    "reference_period": reference_period,
                    "raw_row": {key: str(value) for key, value in normalized.items()},
...
def _parse_road_rows(payload: Any, *, reference_period: str) -> list[dict[str, Any]]:
    ...
    *,
    ...
    for resource in _load_catalog():
        ...
        try:
            ...
            rows = _parse_road_rows(payload, reference_period=reference_period)
            if rows:
                return (
                    rows,
                    raw_for_bronze,
            ...
            warnings.append(f"Urban roads remote resource returned zero rows: {uri}")
        except Exception as exc:
            warnings.append(f"Urban roads remote source failed for '{uri}': {exc}")
    ...
    for candidate in _list_manual_candidates():
        try:
            ...
            warnings.append(f"Urban roads manual source returned zero rows: {candidate.name}")
        except Exception as exc:
            warnings.append(f"Urban roads manual source failed for '{candidate.name}': {exc}")
    return None


def _replace_road_rows(settings: Settings, rows: list[dict[str, Any]]) -> int:
    if not rows:
        return 0
    ...
    with session_scope(settings) as session:
        for source in sources:
            session.execute(
                text("DELETE FROM map.urban_road_segment WHERE source = :source"),
                {"source": source},
            )
        for row in unique_rows:
            session.execute(
                text(
                    """
                    INSERT INTO map.urban_road_segment (
                        source,
                        external_id,
                        name,
                        road_class,
...
def run(
    *,
    ...
    try:
        ...
        if dataset is None:
            warnings.append(
                "No urban roads source available (remote catalog and manual directories failed)."
            )
            ...
            if not dry_run:
                with session_scope(settings) as session:
                    ...
                    replace_pipeline_checks_from_dicts(
                        ...
                        checks=[
                            {
                                "name": "urban_roads_source_resolved",
                                "status": "warn",
                                "details": warnings[-1],
                                "observed_value": 0,
                                "threshold_value": 1,
                            ...
                            {
                                "name": "urban_roads_rows_loaded",
                                "status": "warn",
                                "details": "0 road rows loaded because no source was available.",
                                "observed_value": 0,
                                "threshold_value": 1,
                            },
                        ],
        ...
        rows_written = _replace_road_rows(settings, rows)
        checks = [
            {
                "name": "urban_roads_source_resolved",
            ...
            },
            {
                ...
                "details": f"{rows_written} road rows written into map.urban_road_segment.",
                "observed_value": rows_written,
                "threshold_value": 1,
            },
            {
                "name": "urban_roads_classified_rows",
                "status": "pass" if sum(1 for row in rows if row.get("road_class")) > 0 else "warn",
                "details": "Road rows should include road_class metadata.",
                "observed_value": sum(1 for row in rows if row.get("road_class")),
                "threshold_value": 1,
            },
        ]
        ...
        artifact = persist_raw_bytes(
            ...
            dataset_version="urban-roads-v1",
            checks=checks,
            notes="Urban roads ingestion with bbox-scoped extraction and map upsert.",
            run_id=run_id,
            tables_written=["map.urban_road_segment"] if rows_written > 0 else [],
            rows_written=(
                [{"table": "map.urban_road_segment", "rows": rows_written}]
                if rows_written > 0
                else []
            ),
        )
            ...
            try:
                with session_scope(settings) as session:
                    ...
                    replace_pipeline_checks_from_dicts(
                        ...
                        checks=[
                            {
                                "name": "urban_roads_job_exception",
                                "status": "fail",
                                "details": f"Urban roads connector failed with exception: {exc}",
                                "observed_value": 1,
                                "threshold_value": 0,
                            }
                        ],
