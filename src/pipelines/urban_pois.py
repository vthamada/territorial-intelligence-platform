...
JOB_NAME = "urban_pois_fetch"
SOURCE = "OSM"
DATASET_NAME = "urban_pois_catalog"
WAVE = "MVP-7"
CATALOG_PATH = Path("configs/urban_pois_catalog.yml")
MANUAL_DIRS = (Path("data/manual/urban/pois"), Path("data/manual/urban"))

_HEALTH_AMENITIES = {"hospital", "clinic", "doctors", "dentist", "pharmacy"}
_EDUCATION_AMENITIES = {"school", "college", "university", "kindergarten"}
_SECURITY_AMENITIES = {"police", "fire_station", "courthouse"}
_SOCIAL_AMENITIES = {"social_facility", "community_centre", "shelter"}
_TRANSPORT_AMENITIES = {"bus_station", "ferry_terminal", "parking", "bicycle_parking"}
...
def _load_catalog(path: Path = CATALOG_PATH) -> list[dict[str, Any]]:
    ...
    if not isinstance(resources, list):
        raise ValueError("Invalid urban POIs catalog format: 'resources' must be a list.")
...
def _extract_from_zip(raw_bytes: bytes) -> tuple[bytes, str]:
    with zipfile.ZipFile(io.BytesIO(raw_bytes)) as archive:
        ...
        if not names:
            raise ValueError("ZIP file has no supported POI payload.")
...
def _parse_payload_bytes(raw_bytes: bytes, *, suffix: str) -> tuple[Any, bytes, str]:
    ...
    raise ValueError(f"Unsupported suffix for POI payload: {suffix}")
...
def _classify_poi(tags: dict[str, Any]) -> tuple[str, str | None]:
    amenity = str(tags.get("amenity") or "").strip().casefold()
    healthcare = str(tags.get("healthcare") or "").strip().casefold()
    shop = str(tags.get("shop") or "").strip().casefold()
    tourism = str(tags.get("tourism") or "").strip().casefold()

    if healthcare or amenity in _HEALTH_AMENITIES:
        return "health", healthcare or amenity or None
    if amenity in _EDUCATION_AMENITIES:
        return "education", amenity
    if amenity in _SECURITY_AMENITIES:
        return "security", amenity
    if amenity in _SOCIAL_AMENITIES:
        return "social_assistance", amenity
    if amenity in _TRANSPORT_AMENITIES:
        return "mobility", amenity
    if amenity:
...
def _parse_overpass_rows(payload: dict[str, Any], *, reference_period: str) -> list[dict[str, Any]]:
    ...
    for item in elements:
        ...
        category, subcategory = _classify_poi(tags)
        rows.append(
            {
                "source": "OSM_OVERPASS",
...
def _parse_geojson_rows(payload: dict[str, Any], *, reference_period: str) -> list[dict[str, Any]]:
    ...
    for feature in features:
        ...
        category, subcategory = _classify_poi(properties)
        rows.append(
            {
...
def _parse_tabular_rows(payload: dict[str, Any], *, reference_period: str) -> list[dict[str, Any]]:
    ...
    for item in raw_rows:
        ...
        category, subcategory = _classify_poi(normalized)
        rows.append(
            {
                ...
                "category": str(normalized.get("category") or category or "").strip() or "other",
                "subcategory": str(normalized.get("subcategory") or subcategory or "").strip() or None,
                "metadata_json": {
                    "reference_period": reference_period,
                    "raw_row": properties,
...
def _parse_poi_rows(payload: Any, *, reference_period: str) -> list[dict[str, Any]]:
    ...
    *,
    ...
    for resource in _load_catalog():
        ...
        try:
            ...
            rows = _parse_poi_rows(payload, reference_period=reference_period)
            if rows:
                return rows, raw_for_bronze, normalized_suffix, "remote", uri, Path(uri).name, warnings
            warnings.append(f"Urban POIs remote resource returned zero rows: {uri}")
        except Exception as exc:
            warnings.append(f"Urban POIs remote source failed for '{uri}': {exc}")
    ...
    for candidate in _list_manual_candidates():
        try:
            ...
            warnings.append(f"Urban POIs manual source returned zero rows: {candidate.name}")
        except Exception as exc:
            warnings.append(f"Urban POIs manual source failed for '{candidate.name}': {exc}")
    return None


def _replace_poi_rows(settings: Settings, rows: list[dict[str, Any]]) -> int:
    if not rows:
        return 0
    ...
    with session_scope(settings) as session:
        for source in sources:
            session.execute(
                text("DELETE FROM map.urban_poi WHERE source = :source"),
                {"source": source},
            )
...
def run(
    *,
    ...
    try:
        ...
        if dataset is None:
            warnings.append("No urban POIs source available (remote catalog and manual directories failed).")
            ...
            if not dry_run:
                with session_scope(settings) as session:
                    ...
                    replace_pipeline_checks_from_dicts(
                        ...
                        checks=[
                            {
                                "name": "urban_pois_source_resolved",
                                "status": "warn",
                                "details": warnings[-1],
                                "observed_value": 0,
                                "threshold_value": 1,
                            ...
                            {
                                "name": "urban_pois_rows_loaded",
                                "status": "warn",
                                "details": "0 POI rows loaded because no source was available.",
                                "observed_value": 0,
                                "threshold_value": 1,
                            },
                        ],
        ...
        rows_written = _replace_poi_rows(settings, rows)
        checks = [
            {
                "name": "urban_pois_source_resolved",
            ...
            },
            {
                ...
                "details": f"{rows_written} POI rows written into map.urban_poi.",
                "observed_value": rows_written,
                "threshold_value": 1,
            },
            {
                "name": "urban_pois_categorized_rows",
                "status": "pass" if sum(1 for row in rows if row.get('category')) > 0 else "warn",
                "details": "POI rows should include category metadata.",
                "observed_value": sum(1 for row in rows if row.get("category")),
                "threshold_value": 1,
            },
        ]
        ...
        artifact = persist_raw_bytes(
            ...
            dataset_version="urban-pois-v1",
            checks=checks,
            notes="Urban POIs ingestion with bbox-scoped extraction and map upsert.",
            run_id=run_id,
            tables_written=["map.urban_poi"] if rows_written > 0 else [],
            rows_written=(
                [{"table": "map.urban_poi", "rows": rows_written}]
                if rows_written > 0
                else []
            ),
        )
            ...
            try:
                with session_scope(settings) as session:
                    ...
                    replace_pipeline_checks_from_dicts(
                        ...
                        checks=[
                            {
                                "name": "urban_pois_job_exception",
                                "status": "fail",
                                "details": f"Urban POIs connector failed with exception: {exc}",
                                "observed_value": 1,
                                "threshold_value": 0,
                            }
                        ],
